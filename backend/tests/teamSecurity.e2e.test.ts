...
async function createTenant(label: string) {
  ...
  const site = await api('/api/sites', {
    method: 'POST',
    token,
    body: { name: `${label} site`, domain: `${unique(label)}.test` }
  });
  ...
  const dept = await api('/api/departments', {
    method: 'POST',
    token,
    body: { name: `${label} dept`, siteId: site.body.site._id }
  });
  assert.ok(
    dept.status === 200 || dept.status === 201,
    `department create failed: ${JSON.stringify(dept.body)}`
  );
...
async function createMember(ownerToken: string, role: string, extra: Record<string, unknown> = {}) {
  ...
  const created = await api('/api/team', {
    method: 'POST',
    token: ownerToken,
    body: { name: `${role} member`, email, password: PASSWORD, role, ...extra }
  });
  assert.ok(
    created.status === 200 || created.status === 201,
    `team create failed: ${JSON.stringify(created.body)}`
  );
...
test('an admin cannot promote themselves to owner', async () => {
  ...
  const res = await api(`/api/team/${admin.id}`, {
    method: 'PUT',
    token: admin.token,
    body: { role: 'owner' }
  });
  assert.equal(
    res.status,
    403,
    `self promotion must be refused, got ${res.status} ${JSON.stringify(res.body)}`
  );
...
test('nobody can write a role outside the team role set', async () => {
  ...
  for (const role of ['owner', 'superuser']) {
    const res = await api(`/api/team/${agent.id}`, {
      method: 'PUT',
      token: tenant.token,
      body: { role }
    });
...
test('an admin cannot create or promote a peer admin', async () => {
  ...
  const create = await api('/api/team', {
    ...
    body: {
      name: 'peer',
      email: `${unique('peer')}@team-security.test`,
      password: PASSWORD,
      role: 'admin'
    }
  });
  ...
  const promote = await api(`/api/team/${agent.id}`, {
    method: 'PUT',
    token: admin.token,
    body: { role: 'admin' }
  });
  assert.equal(
    promote.status,
    403,
    `admin promoting to admin must be refused, got ${promote.status}`
  );
  assert.equal(await storedRole(agent.id), 'agent');
});
...
test('an admin cannot edit or delete another admin', async () => {
  ...
  const edit = await api(`/api/team/${other.id}`, {
    method: 'PUT',
    token: admin.token,
    body: { role: 'agent' }
  });
...
test('an admin can still manage agents', async () => {
  ...
  const res = await api(`/api/team/${agent.id}`, {
    method: 'PUT',
    token: admin.token,
    body: { role: 'manager', name: 'Renamed' }
  });
  assert.equal(res.status, 200, JSON.stringify(res.body));
  assert.equal(await storedRole(agent.id), 'manager');
});
...
test("another organization's member status cannot be changed or read", async () => {
  ...
  assert.equal(
    res.body && res.body.email,
    undefined,
    'the response must not carry the foreign profile'
  );
...
test('an agent may change only their own status', async () => {
  ...
  const own = await api(`/api/team/${agent.id}/status`, {
    method: 'PATCH',
    token: agent.token,
    body: { status: 'away' }
  });
  ...
  const other = await api(`/api/team/${colleague.id}/status`, {
    method: 'PATCH',
    token: agent.token,
    body: { status: 'offline' }
  });
...
test("a member cannot be placed into another organization's department", async () => {
  ...
  const res = await api('/api/team', {
    ...
    body: {
      name: 'intruder',
      email: `${unique('intruder')}@team-security.test`,
      password: PASSWORD,
      role: 'agent',
  ...
  const { rows } = await query(
    'SELECT count(*)::int AS n FROM department_members WHERE department_id = $1',
    [victim.department._id]
  );
...
test("another organization's agent cannot be added to a department", async () => {
  ...
  const created = await api('/api/departments', {
    ...
    body: {
      name: 'trap',
      siteId: attacker.site._id,
      members: [{ userId: victimAgent.id, role: 'agent' }]
    }
  });
  assert.equal(created.status, 400, `foreign member must be rejected, got ${created.status}`);
  assert.equal(
    JSON.stringify(created.body || {}).includes('@team-security.test'),
    false,
    'the foreign email must not leak'
  );
  ...
  assert.equal(
    updated.status,
    400,
    `foreign member must be rejected on update, got ${updated.status}`
  );

  const { rows } = await query(
    'SELECT count(*)::int AS n FROM team_departments WHERE team_id = $1',
    [victimAgent.id]
  );
  assert.equal(rows[0].n, 0, "the victim agent's record must be untouched");
  const { rows: memberRows } = await query(
    'SELECT count(*)::int AS n FROM department_members WHERE user_id = $1',
    [victimAgent.id]
  );
...
test("another organization's agent cannot be added through the single-member route", async () => {
  ...
  const res = await api(`/api/departments/${attacker.department._id}/members`, {
    method: 'POST',
    token: attacker.token,
    body: { userId: victimAgent.id, role: 'agent' }
  });
...
async function seedConversation(
  site: { _id: string; organizationId: string },
...
test('an agent cannot delete a conversation; the owner can', async () => {
  ...
  const byAgent = await api(`/api/conversations/${tenant.site._id}/${conversationId}`, {
    method: 'DELETE',
    token: agent.token
  });
  assert.equal(byAgent.status, 403, `agent delete must be refused, got ${byAgent.status}`);
  const { rows } = await query('SELECT count(*)::int AS n FROM conversations WHERE id = $1', [
    conversationId
  ]);
  assert.equal(rows[0].n, 1, 'the conversation must survive');

  const byOwner = await api(`/api/conversations/${tenant.site._id}/${conversationId}`, {
    method: 'DELETE',
    token: tenant.token
  });
...
test('an agent may take an unassigned conversation or release their own, nothing more', async () => {
  ...
  const byOwner = await api(`/api/conversations/${theirs}/assign`, {
    method: 'PUT',
    token: tenant.token,
