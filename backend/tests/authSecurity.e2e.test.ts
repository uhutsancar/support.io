...
async function call(path: string, init: RequestInit = {}) {
  ...
  let body: any = null;
  try { body = await res.json(); } catch (e) { /* boş gövde */ }
...
const sessionKey = () =>
  crypto.createHmac('sha256', String(process.env.JWT_SECRET)).update('support-chat/session/v1').digest();
...
test('an upload proof is never accepted as a session', async () => {
  const proof = signUploadProof({
    kind: 'chat-upload', siteId: '000000000000000000000000',
    filename: 'x.png', url: 'https://example.test/x.png', size: 1, mimeType: 'image/png'
  });
...
test('a token signed the old way (raw secret, no audience) is rejected', async () => {
  ...
  const legacy = jwt.sign({ userId: String(user._id), userType: 'user' }, String(process.env.JWT_SECRET), { expiresIn: '5m' });
...
test('a session-keyed token without a user id is rejected', async () => {
  const token = jwt.sign({ userType: 'user' }, sessionKey(), {
    algorithm: 'HS256', audience: 'support-chat:session', expiresIn: '5m'
  });
...
test('an undefined filter value throws instead of widening the query', async () => {
  await assert.rejects(
    async () => { await User.findOne({ _id: undefined, isActive: true }); },
...
test('login returns no token in the body and sets an httpOnly session cookie', async () => {
  ...
  assert.doesNotMatch(cookies.sc_csrf.raw, /HttpOnly/i, 'the csrf pair must be readable by the panel');
...
test('a cookie-authenticated write needs the matching CSRF header', async () => {
  ...
  const write = (extra: Record<string, string>) => call('/api/auth/status', {
    method: 'PUT',
  ...
  assert.equal((await write({ 'X-CSRF-Token': cookies.sc_csrf.value })).status, 200, 'matching header');
...
test('logout clears the session cookie', async () => {
  ...
  const out = await call('/api/auth/logout', {
    method: 'POST', headers: { Cookie: cookieHeader, 'X-CSRF-Token': cookies.sc_csrf.value }
  });
...
test('a query-string operator cannot pick an arbitrary site', async () => {
  ...
  assert.notEqual(res.status, 200, `settings answered for an operator: ${JSON.stringify(res.body).slice(0, 120)}`);
...
test('a JSON-body operator cannot write to an arbitrary site', async () => {
  ...
  const { rows } = await query("SELECT count(*)::int AS n FROM sites WHERE installation->>'origin' = $1", [new URL(marker).origin]);
