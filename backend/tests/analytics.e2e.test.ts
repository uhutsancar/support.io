...
/** One synthetic conversation, shaped so the dashboards can be asserted on. */
...
async function seedConversation({
  ...
  ratingScore = null,
  slaStatus = 'pending'
}: SeedConversationSpec): Promise<string> {
  const id = generateId();
...
test('the aggregate covers more rows than the conversation list endpoint returns', async (t) => {
  ...
  // The list endpoint returns one page, which is what the client-side
  // aggregation used to read and why its figures were wrong.
  const list = await api(`/api/conversations/${tenant.site._id}`, { token: tenant.token });
  assert.equal(list.status, 200);
  assert.ok(
    list.body.conversations.length < ids.length,
    'a single page is expected to be smaller than the full set'
  );
  assert.ok(list.body.hasMore, 'a truncated page must say more rows exist');
  assert.ok(list.body.nextCursor, 'a truncated page must offer a cursor to continue');

  // Unlike the old hard cap, the whole set is now reachable by paging.
  const walked = new Set(list.body.conversations.map((c: any) => c._id));
  let cursor = list.body.nextCursor;
  for (let guard = 0; cursor && guard < 20; guard++) {
    const next = await api(
      `/api/conversations/${tenant.site._id}?cursor=${encodeURIComponent(cursor)}`,
      { token: tenant.token }
    );
    assert.equal(next.status, 200);
    next.body.conversations.forEach((c: any) => walked.add(c._id));
    cursor = next.body.nextCursor;
  }
  assert.equal(walked.size, ids.length, 'paging must reach every conversation exactly once');
...
test('overview figures agree with the rows behind them', async (t) => {
  ...
  const statusTotal = byStatus.reduce((sum: any, r: any) => sum + r.value, 0);
  assert.equal(statusTotal, 3, 'status breakdown must account for every conversation');

  const priorityTotal = byPriority.reduce((sum: any, r: any) => sum + r.value, 0);
  assert.equal(priorityTotal, 3);

  const channelTotal = channelDistribution.reduce((sum: any, r: any) => sum + r.value, 0);
...
test('agent headcount tiles are real, not the old hardcoded 4 and 5', async (t) => {
  ...
  const ada = res.body.agentPerformance.find((a: any) => a._id === onlineId);
