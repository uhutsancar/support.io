...
const { AIProvider, AIError } = require('./provider');

// The SDK ships as ESM with a CommonJS interop default export; this project is
// CommonJS, so the default has to be unwrapped explicitly.
...
class AnthropicProvider extends AIProvider {
  constructor({ apiKey, model = DEFAULT_MODEL, timeoutMs = 30000 } = {}) {
    super();
    this.model = model;
    this.apiKey = apiKey;
  ...
  async complete({ system, prompt, maxTokens = DEFAULT_MAX_TOKENS, effort = 'low' } = {}) {
    if (!this.client) {
  ...
  translateError(error) {
    ...
    return new AIError(error.message || 'Bilinmeyen AI hatası.', { code: 'ai_error' });
  }
}
