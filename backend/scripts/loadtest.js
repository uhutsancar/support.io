...
async function scenarioPanel(token, siteId) {
  const samples = [];
  let errors = 0;
...
function visitorSession(siteKey, index) {
  return new Promise((resolve) => {
    const started = Date.now();
    const visitorId = `load-${Date.now()}-${index}`;
    const result = { joinMs: null, sendMs: null, ok: false, error: null };
...
async function scenarioWidget(siteKey) {
  const joins = [];
  const sends = [];
  let errors = 0;
...
async function main() {
  ...
  const site = sites.find((s) => s.siteKey === 'demo-site-key-0000-1111-2222') || sites[0];
