...
const Dots = ({ color }: { color?: string; [prop: string]: any }) => (
...
const StudioPreview = ({
  ...
  const launcherSize = SIZE_PX[button.size as keyof typeof SIZE_PX] || 60;
  const launcherRadius = button.borderRadius >= 50 ? '50%' : `${button.borderRadius}%`;
  const iconPath =
    LAUNCHER_ICONS[button.icon as keyof typeof LAUNCHER_ICONS] ||
    LAUNCHER_ICONS['message-circle' as keyof typeof LAUNCHER_ICONS];
  ...
  const statusText =
    availability === 'online'
      ...
      : availability === 'away'
        ...
        : strings.offline;
  const statusDot =
    availability === 'online' ? '#22C55E' : availability === 'away' ? '#F59E0B' : '#9CA3AF';
        ...
        {messages.welcomeMessage ? (
          <div className="flex flex-col gap-1 self-start max-w-[82%]">
            ...
            {messages.showTimestamps !== false && (
              <span className="text-[10.5px] px-1" style={{ color: colors.textSecondary }}>
                09:24
              </span>
            )}
          </div>
      ...
      <span
        ...
        style={{
          ...
          boxShadow:
            button.shadow === false
              ? 'none'
              : `0 8px 24px ${colors.primary}52, 0 2px 6px rgba(0,0,0,.12)`
        }}
      >
        <svg
          viewBox="0 0 24 24"
