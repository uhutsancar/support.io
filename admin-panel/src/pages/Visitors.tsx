...
import type { Site, Visitor } from '../types/api';
...
const Visitors = () => {
  ...
  useEffect(() => {
    if (!activeSite || !socket) return undefined;
    socket.emit('join-site', { siteId: activeSite });
    const onVisitorUpdated = (doc: any) => {
      setVisitors((prev) => {
        const index = prev.findIndex((v) => v.visitorId === doc.visitorId);
        if (doc.isActive && (!startTimes.current.has(doc.visitorId) || index === -1)) {
          startTimes.current.set(doc.visitorId, Date.now());
        } else if (!doc.isActive) {
          startTimes.current.delete(doc.visitorId);
        }
        const nextState = [...prev];
        if (index > -1) {
          if (!doc.isActive) nextState.splice(index, 1);
          else nextState[index] = doc;
        } else if (doc.isActive) {
    ...
    socket.on('visitor-updated', onVisitorUpdated);
    return () => {
      socket.off('visitor-updated', onVisitorUpdated);
    };
  }, [activeSite, socket]);
  useEffect(() => {
  ...
  const loadVisitors = async () => {
    try {
      setLoading(true);
      const response = await visitorsAPI.getAll(activeSite as string, true);
      ...
      console.error('[visitors] request failed', error);
    } finally {
      setLoading(false);
    }
  };
  const formatDuration = (visitorId: string) => {
  ...
  const getDeviceIcon = (os: any, browser: any) => {
  ...
  const getFormattedName = (visitorId: string) => {
    return visitorId
      ? visitorId.substring(0, 8).toUpperCase()
      : t('visitors.unknown', 'BİLİNMEYEN');
  };
  return (
    <>
      <Helmet>
              ...
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-800 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                <tr>
                  <th scope="col" className="px-6 py-4">
                    {t('visitors.timeOnSite', 'Sitede Geçen Süre')}
                  </th>
                  <th scope="col" className="px-6 py-4">
                    {t('visitors.visitorColumn', 'Ziyaretçi')}
                  </th>
                  <th scope="col" className="px-6 py-4 hidden md:table-cell">
                    {t('visitors.currentPage', 'Geçerli Sayfa')}
                  </th>
                  <th scope="col" className="px-6 py-4 hidden sm:table-cell">
                    {t('visitors.locationIp', 'Konum / IP')}
                  </th>
                  <th scope="col" className="px-6 py-4 hidden lg:table-cell">
                    {t('visitors.system', 'Sistem')}
                  </th>
                </tr>
              </thead>
              <tbody>
                {visitors.map((visitor) => (
                  <tr
                    key={visitor._id}
                    className="bg-white dark:bg-gray-900 border-b dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors animate-fade-in"
                  >
                    <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                        <span className="font-mono text-indigo-600 dark:text-indigo-400">
                          {formatDuration(visitor.visitorId)}
                        </span>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center flex-shrink-0">
                          <span className="text-indigo-600 dark:text-indigo-400 font-bold text-xs">
                            {getFormattedName(visitor.visitorId).charAt(0)}
                          </span>
                        </div>
                        <div>
                          <div className="font-semibold text-gray-900 dark:text-white">
                            {t('visitors.idPrefix', 'Kimlik:')}{' '}
                            {getFormattedName(visitor.visitorId)}
                          </div>
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            {visitor.ip || t('visitors.unknown', 'Bilinmiyor')}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 hidden md:table-cell">
                      <div className="flex items-center gap-2 max-w-[200px]">
                        ...
                        <span className="truncate" title={visitor.currentPage}>
                          {visitor.currentPage}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 hidden sm:table-cell">
                      <div className="flex flex-col">
                        <span className="text-gray-900 dark:text-gray-300">
                          {visitor.country || t('visitors.unknown', 'Bilinmiyor')}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 hidden lg:table-cell">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="capitalize">
                          {visitor.os || visitor.browser || t('visitors.unknown', 'Bilinmiyor')}
                        </span>
                      </div>
                    </td>
                  </tr>
