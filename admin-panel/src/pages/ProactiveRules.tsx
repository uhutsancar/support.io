...
const ProactiveRules = () => {
  ...
  const toggleStatus = async (rule: any) => {
    try {
      const res = await api.put(`/proactive-rules/${rule._id}`, {
        ...rule,
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          {t('proactive.selectSite', 'Select Site')}
        </label>
        <select
            ...
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('proactive.tableRuleName', 'Rule Name')}
                </th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('proactive.tableTrigger', 'Trigger')}
                </th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('proactive.tableAction', 'Action')}
                </th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('proactive.tableConversions', 'Conversions')}
                </th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('proactive.tableStatus', 'Status')}
                </th>
                <th className="px-6 py-4 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('proactive.tableActions', 'Actions')}
                </th>
              </tr>
            </thead>
              ...
              {rules.length === 0 ? (
                <tr>
                  ...
                  >
                    {t(
                      'proactive.emptyText',
                      'No active rules for this site. Create one to get started!'
                    )}
                  </td>
                </tr>
              ) : (
                rules.map((rule) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                      ...
                      {rule.triggerCondition.eventType === 'time_on_page' &&
                        ` (${rule.triggerCondition.timeThresholdSeconds}s)`}
                    </td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50 rounded-t-xl">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                {editingRule
                  ? t('proactive.editRule', 'Edit Proactive Rule')
                  : t('proactive.newRule', 'New Proactive Rule')}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div>
                <label className="block text-sm font-medium mb-1 dark:text-gray-300">
                  {t('proactive.ruleNameLabel', 'Rule Name')}
                </label>
                <input
                  required
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">
                    {t('proactive.eventTriggerLabel', 'Event Trigger')}
                  </label>
                  <select
                    ...
                    className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 outline-none dark:text-white"
                  >
                    <option value="time_on_page">
                      {t('proactive.triggerTime', 'Time on Page')}
                    </option>
                    <option value="exit_intent">{t('proactive.triggerExit', 'Exit Intent')}</option>
                    <option value="scroll_depth">
                      {t('proactive.triggerScroll', 'Scroll Depth')}
                    </option>
                    <option value="inactivity">
                      {t('proactive.triggerInactivity', 'Inactivity')}
                    </option>
                  </select>
                </div>

                {(formData.triggerCondition.eventType === 'time_on_page' ||
                  formData.triggerCondition.eventType === 'inactivity') && (
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">
                      {t('proactive.secondsLabel', 'Seconds')}
                    </label>
                    <input
                      type="number"
                      min="1"
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...
                          triggerCondition: {
                            ...formData.triggerCondition,
                            timeThresholdSeconds: parseInt(e.target.value)
                          }
                        })
                      }
                      className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 outline-none dark:text-white"
                ...
                {formData.triggerCondition.eventType === 'scroll_depth' && (
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">
                      {t('proactive.scrollLabel', 'Scroll %')}
                    </label>
                    <input
                      type="number"
                      min="10"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">
                    {t('proactive.urlMatchLabel', 'URL Match')}
                  </label>
                  <select
                ...
                {formData.triggerCondition.urlMatch !== 'any' && (
                  <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">
                      {t('proactive.urlValueLabel', 'URL Value')}
                    </label>
                    <input
                      type="text"
              ...
              <div>
                <label className="block text-sm font-medium mb-1 dark:text-gray-300">
                  {t('proactive.actionLabel', 'Action')}
                </label>
                <select
                ...
                >
                  <option value="open_popup">
                    {t('proactive.actOpenPopup', 'Open Widget Window')}
                  </option>
                  <option value="send_message">
                    {t('proactive.actSendMessage', 'Send Chat Message')}
                  </option>
                </select>
