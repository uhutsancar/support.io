...
const Team = () => {
  ...
  useEffect(() => {
    ...
    socket.on('team-member-updated', onMemberUpdated);
    socket.on('team-member-deleted', onMemberDeleted);
    return () => {
      socket.off('agent-status-changed', onAgentStatusChanged);
      socket.off('team-member-added', onMemberAdded);
      socket.off('team-member-updated', onMemberUpdated);
      socket.off('team-member-deleted', onMemberDeleted);
    };
  }, [socket, t]);
  useEffect(() => {
    fetchSites();
  ...
  useEffect(() => {
    if (selectedSite) {
      fetchTeam();
      socket?.emit('join-site', { siteId: selectedSite });
    }
  }, [selectedSite, socket]);
