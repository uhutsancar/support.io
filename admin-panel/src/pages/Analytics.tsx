...
const Analytics = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      setStats({
        ...
        slaBreaches: figures.slaBreaches,
        unassigned: figures.unassigned,
        satisfaction: figures.satisfaction,
        activeAgents: figures.activeAgents,
        totalAgents: figures.totalAgents,
        totalConversations: figures.totalConversations,
        resolvedConversations: figures.resolvedConversations,
      ...
      setAgentPerformance(
        data.agentPerformance.map((a: any) => ({
          ...a,
          satisfaction: a.rating === null || a.rating === undefined ? 0 : Math.round(a.rating * 20)
        }))
      );
    } catch (err) {
  ...
  return (
    <>
      <Helmet>
        <title>{`${t('analytics.title') || ''} — Support.io`}</title>
