...
import { Send, Search, Users, MessageSquare, Plus, User, X, ChevronLeft } from 'lucide-react';
import ConfirmDialog from '../components/ConfirmDialog';
import type { TeamChat, TeamChatMessage, TeamChatParticipant } from '../types/api';
...
const TeamChatPage = () => {
  const loadChats = async () => {
    try {
      ...
      console.error('[team-chat] request failed', error);
    }
  };
  ...
  const [members, setMembers] = useState<TeamChatParticipant[]>([]);
  ...
  useEffect(() => {
    ...
    const onMessage = (data: any) => {
      setMessages((prev) => {
        if (String(data.message.chatId) !== String(activeChat?.chatId || '')) return prev;
    ...
    const onTyping = (data: any) => {
      ...
      remoteTypingTimeout.current = setTimeout(() => setTypingUser(null), 2000);
    };
    socket.on('team-chat-message', onMessage);
    socket.on('team-chat-user-typing', onTyping);
    return () => {
      socket.off('team-chat-message', onMessage);
      socket.off('team-chat-user-typing', onTyping);
      if (remoteTypingTimeout.current) clearTimeout(remoteTypingTimeout.current);
    };
  ...
  const getChatName = (chat: any) => {
    ...
    const other = chat.participants?.find((p: any) => p && p._id !== currentUserId);
  ...
  const statusText = (status: string) => {
    switch (status) {
      case 'online':
        return t('teamChat.online') || 'Online';
      case 'away':
        return t('teamChat.away') || 'Away';
      case 'busy':
        return t('teamChat.busy') || 'Busy';
      default:
        return t('teamChat.offline') || 'Offline';
    }
  };
  // The chat-style timestamp lives in lib/format.ts; the translated label is
  // passed in because that module has no access to `t`.
  const formatTime = (date: string | Date | null | undefined) =>
    formatChatTimestamp(date, t('teamChat.yesterday', 'Yesterday'));
  return (
    <div className="h-[calc(100vh-3rem)] flex flex-col">
      ...
      <div className="flex-1 flex bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden min-h-0">
        {}
        <div
          className={`w-full md:w-80 lg:w-96 border-r border-gray-200 dark:border-gray-700 flex flex-col ${mobileView !== 'list' ? 'hidden md:flex' : 'flex'}`}
        >
          {}
            ...
            {filteredChats.length === 0 ? (
              ...
              filteredChats.map((chat) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition text-left ${
                ...
                >
                  <div className="relative flex-shrink-0">
                    ...
                    {getChatStatus(chat) && (
                      <div
                        className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white dark:border-gray-800 ${statusColor(getChatStatus(chat))}`}
                      />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
        ...
        {}
        <div
          className={`flex-1 flex flex-col min-w-0 ${mobileView !== 'chat' ? 'hidden md:flex' : 'flex'}`}
        >
          {activeChat ? (
            <>
              {}
                ...
                {activeChat.chatType === 'group' ? (
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    {getChatStatus(activeChat) && (
                      <div
                        className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white dark:border-gray-800 ${statusColor(getChatStatus(activeChat))}`}
                      />
                    )}
                  </div>
                )}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50 dark:bg-gray-900/50 modal-scrollbar pr-2">
                ...
                {typingUser && (
                  <div className="flex items-center gap-2 text-gray-400 text-xs">
                    ...
                    <span>
                      {typingUser} {t('teamChat.isTyping')}
                    </span>
                  </div>
                )}
        ...
        {}
        <div
          className={`w-64 border-l border-gray-200 dark:border-gray-700 flex-col bg-white dark:bg-gray-800 ${
            activeChat?.chatType === 'group'
              ? mobileView === 'members'
                ? 'flex'
                : 'hidden lg:flex'
                ...
                <div key={status} className="mb-3">
                  ...
                  {statusMembers.map((m) => (
                    <button
                      key={m._id}
                      onClick={() => {
                        if (m._id !== (user?._id || user?.id)) startDirectChat(m);
                      }}
                    ...
                    >
                      <div className="relative">
                        ...
                        <div
                          className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white dark:border-gray-800 ${statusColor(m.status)}`}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-gray-800 dark:text-gray-200 truncate">
                          {m.name}{' '}
                          {m._id === (user?._id || user?.id) && (
                            <span className="text-gray-400">({t('teamChat.you')})</span>
                          )}
                        </p>
