...
import { Plus, GripVertical, UserCircle, Briefcase, Mail, X, Trash2, Tag } from 'lucide-react';
import toast from 'react-hot-toast';
import ConfirmDialog from '../components/ConfirmDialog';
import type { Deal, DealForm } from '../types/api';

const STAGES = [
  {
    id: 'new',
    labelKey: 'crm.stages.new',
    color: 'border-blue-500',
    bgInfo: 'bg-blue-50 dark:bg-blue-900/20'
  },
  {
    id: 'potential',
    labelKey: 'crm.stages.potential',
    color: 'border-purple-500',
    bgInfo: 'bg-purple-50 dark:bg-purple-900/20'
  },
  {
    id: 'quoted',
    labelKey: 'crm.stages.quoted',
    color: 'border-yellow-500',
    bgInfo: 'bg-yellow-50 dark:bg-yellow-900/20'
  },
  {
    id: 'negotiation',
    labelKey: 'crm.stages.negotiation',
    color: 'border-orange-500',
    bgInfo: 'bg-orange-50 dark:bg-orange-900/20'
  },
  {
    id: 'won',
    labelKey: 'crm.stages.won',
    color: 'border-green-500',
    bgInfo: 'bg-green-50 dark:bg-green-900/20'
  },
  {
    id: 'lost',
    labelKey: 'crm.stages.lost',
    color: 'border-red-500',
    bgInfo: 'bg-red-50 dark:bg-red-900/20'
  }
];
const CRM = () => {
  ...
  const calculateFormatValue = (val: any) => {
  ...
  const handleDragStart = (e: React.DragEvent, deal: any) => {
    ...
    setTimeout(() => {
      if (e.target) (e.target as HTMLElement).classList.add('opacity-50');
    }, 0);
  };
  ...
  const handleDrop = async (e: React.DragEvent, targetStageId: any) => {
    e.preventDefault();
    setDragOverColumn(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { data } = await dealsAPI.create({
        ...formData,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">
                {t('crm.title', 'CRM - Fırsatlar')}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {t('crm.subtitle', 'Satış aşamalarınızı sürükle bırak yönetin')}
              </p>
            </div>
          </div>
          <button
          ...
          <div className="flex gap-6 h-full items-start min-w-max pb-4">
            {STAGES.map((stage) => (
              ...
              >
                {}
                <div
                  className={`p-4 border-t-4 rounded-t-xl border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 ${stage.color} flex justify-between items-center sticky top-0`}
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {calculateFormatValue(getStageTotal(stage.id))}
                    </p>
                  </div>
                  <button
                      ...
                      .map((deal) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-2">
                            <h4 className="font-medium text-gray-900 dark:text-white leading-tight pr-6">
                              {deal.title}
                            </h4>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg border border-gray-200 dark:border-gray-700 animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white/90">
                  {t('crm.modalTitle', 'Yeni Fırsat Oluştur')}
                </h3>
                <button
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="pt-2 border-t border-gray-100 dark:border-gray-700/50">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 mt-2">
                    {t('crm.formCustomerDetails', 'Müşteri Detayları')}{' '}
                ...
                <div className="flex justify-end gap-3 pt-4 font-medium">
                  ...
                  <button
                    type="submit"
                    className="px-6 py-2 text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition shadow-md"
                  >
