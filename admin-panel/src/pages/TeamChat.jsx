...
const TeamChatPage = () => {
  ...
  // Mesaj silme fonksiyonu (en üstte, return dışında olmalı)
  const handleDeleteMessage = async () => {
    if (!confirmDialog.messageId) return;
    try {
      // API isteği ile mesajı sil (örnek endpoint, backend'e göre güncellenmeli)
  ...
  const [mobileView, setMobileView] = useState('list'); // 'list' | 'chat' | 'members'
  ...
  useEffect(() => {
    ...
    // Her bağlantı değişiminde de join-site gönder
    if (user?._id) {
      newSocket.emit('join-site', { userId: user._id });
    } else if (user?.id) {
  ...
  const sendMessage = () => {
    ...
    console.log('Sending team-chat-send', { chatId: activeChat.chatId, content: newMessage.trim(), chatType: activeChat.chatType });
  ...
  // Join/leave room and load messages when activeChat changes
  useEffect(() => {
    if (!socket) return;
    let prevChatId = null;
  ...
  return (
    <div className="h-[calc(100vh-3rem)] flex flex-col">
      ...
      <div className="flex-1 flex bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden min-h-0">
              ...
              filteredChats.map(chat => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition text-left ${
                    activeChat?.chatId === chat.chatId ? 'bg-indigo-50 dark:bg-indigo-900/20 border-l-2 border-l-indigo-500' : ''
                  }`}
                >
        ...
        {/* RIGHT: Members Panel (visible on desktop when group selected, or mobile toggle) */}
        <div className={`w-64 border-l border-gray-200 dark:border-gray-700 flex-col bg-white dark:bg-gray-800 ${
          activeChat?.chatType === 'group' 
              ...
              return (
                <div key={status} className="mb-3">
                  ...
                  {statusMembers.map(m => (
                    <button
                      key={m._id}
                      onClick={() => { if (m._id !== user?._id) startDirectChat(m); }}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-gray-800 dark:text-gray-200 truncate">
                          {m.name} {m._id === user?._id && <span className="text-gray-400">({t('teamChat.you')})</span>}
                        </p>
