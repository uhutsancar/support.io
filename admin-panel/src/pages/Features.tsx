...
import {
  ...
  Sparkles, Eye, Briefcase, ArrowUpRight, Check
} from 'lucide-react';
import Shell, { PageHero, useMarketingRoutes } from '../components/marketing/Shell';
import {
  Button, TextLink, Section, SectionHead, Card, AccentIcon, Pill, AppFrame,
  accent, asList
} from '../components/marketing/kit';
import {
  InboxVisual, AnalyticsVisual, AutomationVisual, KnowledgeVisual, TeamVisual
} from '../components/marketing/visuals';
import {
  FEATURE_GROUPS, FEATURE_ICON, FEATURE_PLAN, FEATURE_TONE
...
const GROUP_VISUAL = {
  talk: (t: any) => <AppFrame label={t('viz.inbox.frame')}><InboxVisual compact /></AppFrame>,
  organize: (t: any) => <AppFrame label={t('viz.automation.frame')} tone="emerald"><AutomationVisual /></AppFrame>,
  grow: (t: any) => <AppFrame label={t('viz.analytics.frame')} tone="sky"><AnalyticsVisual /></AppFrame>
};

const FeatureCard = ({ id, routes, t }: { id?: string; routes?: any; t?: any; [prop: string]: any }) => {
  const Icon = ICONS[FEATURE_ICON[id as keyof typeof FEATURE_ICON] as keyof typeof ICONS] || MessageSquare;
  const badge = PLAN_BADGE[FEATURE_PLAN[id as keyof typeof FEATURE_PLAN] as keyof typeof PLAN_BADGE];
    ...
    <Shell>
      ...
      >
        <div className="mt-9 flex flex-wrap gap-3">
          ...
          <Button to={routes.pricing} variant="secondary" size="lg">{t('landing.home.ctaBtn2')}</Button>
        </div>
      </PageHero>
          ...
          <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] gap-10 lg:gap-16 items-center">
            <div className={gi % 2 === 1 ? 'lg:order-2' : ''}>
              ...
              <ul className="mt-7 space-y-3">
                {asList(t('featuresPage.groups.' + group.id + '.points', { returnObjects: true })).map((p, i) => (
                  <li key={i} className="flex gap-2.5">
                    <Check className={['w-4 h-4 mt-1 shrink-0', accent(group.tone).text].join(' ')} />
                    <span className="text-[14.5px] leading-relaxed text-gray-600 dark:text-gray-400">{p}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] gap-10 lg:gap-16 items-start">
          ...
          <div className="grid sm:grid-cols-2 gap-x-8 gap-y-7">
            ...
            <div className="sm:col-span-2">
              <TextLink to={routes.docs} tone="violet">{t('featuresPage.readDocs')}</TextLink>
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section tone="subtle" bordered>
        <div className="text-center max-w-2xl mx-auto">
          <h2 className="text-[30px] sm:text-[36px] font-semibold tracking-[-0.03em]
