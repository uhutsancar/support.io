...
import type { FAQ, Site } from '../types/api';
...
const FAQs = () => {
  ...
  const fetchSites = async () => {
    try {
      ...
      console.error('[faqs] request failed', error);
    }
  };
  const fetchFAQs = async (siteId: string) => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(errorMessage(error, 'FAQ kaydedilemedi'));
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(errorMessage(error, t('faqs.deleteError')));
    }
  };
  const openDeleteConfirm = (faqId: string, faqQuestion: any) => {
  ...
  return (
    <>
      <Helmet>
        <title>{`${t('faqs.title') || ''} — Support.io`}</title>
