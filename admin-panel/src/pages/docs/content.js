...
export const embedSnippet = (origin, siteKey = 'YOUR_SITE_KEY') =>
...
<html>
  ...
  },
  {
    ...
    lang: 'jsx',
    code: (origin, key) => `// 1) En basit yol: public/index.html içine script etiketini koyun.
...
// effect iki kez çalışsa bile ikinci bir widget oluşmaz — runtime singleton'dır.`
  },
  {
    ...
    lang: 'tsx',
    code: (origin, key) => `import Script from 'next/script';
...
// üretmez. next/script "afterInteractive" ile ilk boyamayı geciktirmez.`
  },
  {
    ...
    lang: 'vue',
    code: (origin, key) => `<!-- index.html içine script etiketini koymak yeterlidir. -->
...
watch(() => props.user, (user) => {
  ...
  },
  {
    ...
    lang: 'ts',
    code: (origin, key) => `// .client.ts uzantısı önemlidir: eklenti yalnızca tarayıcıda çalışır.
...
//   'data-site-key': '${key}' }] } }`
  },
  {
    ...
    lang: 'ts',
    code: (origin, key) => `<!-- src/index.html, </body> öncesi -->
...
export class AppComponent implements OnInit {
  ...
  },
  {
    ...
    lang: 'svelte',
    code: (origin, key) => `<!-- SvelteKit: src/app.html içinde %sveltekit.body% sonrasına -->
<script src="${origin}/widget.js" data-site-key="${key}" async></script>
...
add_action('wp_footer', function () {
  ...
  },
  {
    ...
    lang: 'blade',
    code: (origin, key) => `{{-- </body> etiketinden hemen önce --}}
...
<script src="<?= htmlspecialchars($supportChatUrl, ENT_QUOTES) ?>/widget.js"
  ...
  },
  {
    ...
    lang: 'liquid',
    code: (origin, key) => `{%- comment -%} </body> etiketinden hemen önce {%- endcomment -%}
<script src="${origin}/widget.js" data-site-key="${key}" async></script>
