...
// Mirrors AuditAction on the server: an action missing here cannot be filtered
// for, which is how the automation entries stayed out of reach.
const ACTIONS = [
  '',
  'LOGIN_SUCCESS',
  'LOGIN_FAILED',
  ...
  switch (action) {
    case 'LOGIN_SUCCESS':
      return {
        icon: ShieldCheck,
        color:
          'text-green-600 bg-green-50 dark:text-green-400 dark:bg-green-500/10 border-green-200 dark:border-green-500/20'
      };
    case 'LOGIN_FAILED':
      return {
        icon: ShieldAlert,
        color:
          'text-red-600 bg-red-50 dark:text-red-400 dark:bg-red-500/10 border-red-200 dark:border-red-500/20'
      };
    case 'CREATE_AGENT':
      return {
        icon: UserPlus,
        color:
          'text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-500/10 border-blue-200 dark:border-blue-500/20'
      };
    case 'DELETE_AGENT':
      return {
        icon: UserMinus,
    ...
    case 'UPDATE_AGENT_ROLE':
      return {
        icon: UserCog,
        color:
          'text-purple-600 bg-purple-50 dark:text-purple-400 dark:bg-purple-500/10 border-purple-200 dark:border-purple-500/20'
      };
    case 'PLAN_CHANGED':
      return {
        icon: CreditCard,
    ...
    case 'UPDATE_SLA':
      return {
        icon: Settings,
        color:
          'text-slate-600 bg-slate-50 dark:text-slate-400 dark:bg-slate-500/10 border-slate-200 dark:border-slate-500/20'
      };
    case 'TICKET_CLOSED':
      return {
        icon: Ticket,
        color:
          'text-indigo-600 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-500/10 border-indigo-200 dark:border-indigo-500/20'
      };
    case 'TICKET_REOPENED':
      return {
        icon: RefreshCcw,
        color:
          'text-teal-600 bg-teal-50 dark:text-teal-400 dark:bg-teal-500/10 border-teal-200 dark:border-teal-500/20'
      };
    case 'SLA_BREACH':
      return {
        icon: AlertCircle,
        color:
          'text-orange-600 bg-orange-50 dark:text-orange-400 dark:bg-orange-500/10 border-orange-200 dark:border-orange-500/20'
      };
    default:
      return {
        icon: Activity,
        color:
          'text-gray-600 bg-gray-50 dark:text-gray-400 dark:bg-gray-500/10 border-gray-200 dark:border-gray-500/20'
      };
  }
};
...
export default function AuditLogs() {
  ...
  // Anahtar calisma aninda olustugu icin i18next 'string | object' donuyor;
  // bu sozlukteki tum karsiliklar metin, o yuzden burada daraltiyoruz.
  const label = (key: string, fallback: string): string => String(t(key, fallback));
  ...
  const toggleExpand = (id: string) => {
    setExpanded((prev) => ({ ...prev, [id]: !prev[id as keyof typeof prev] }));
  };
  ...
  const copyToClipboard = async (id: string, text: string) => {
    try {
      ...
      toast.success(t('audit.copySuccess', 'Kopyalandı'));
    } catch {
        ...
        <div className="overflow-x-auto min-h-[400px]">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50/50 dark:bg-gray-800/50 backdrop-blur-sm shadow-sm sticky top-0 z-10">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-40">
                  {t('audit.time')}
                </th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('audit.action')}
                </th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden sm:table-cell">
                  {t('audit.entity')}
                </th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden md:table-cell">
                  {t('audit.user')} & {t('audit.ip')}
                </th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-1/3">
                  {t('audit.metadata')}
                </th>
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {loading ? (
                ...
                docs.map((d) => {
                  ...
                  const isExpanded = expanded[d._id as keyof typeof expanded];
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div
                          className={`inline-flex items-center gap-2 px-2.5 py-1.5 rounded-md border ${conf.color}`}
                        >
                          <Icon className="w-4 h-4 shrink-0" />
                          <span className="text-xs font-semibold whitespace-nowrap">
                            {label('audit.actions.' + d.action, d.action)}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 hidden sm:table-cell">
                        <div className="flex flex-col">
                          <span className="font-medium">
                            {d.entityType
                              ? label(`audit.entities.${d.entityType}`, d.entityType)
                              : '-'}
                          </span>
                          <span
                            className="text-xs text-gray-400 dark:text-gray-500 break-words max-w-[150px] truncate"
                            title={d.entityId}
                          >
                            {d.entityId || '-'}
                          </span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 hidden md:table-cell">
                        <div className="flex flex-col">
                          <span className="font-medium truncate max-w-[150px]" title={d.userId}>
                            {d.userId || 'System'}
                          </span>
                          ...
                          <div className="bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                            ...
                            >
                              <span className="flex items-center gap-1.5">
                                ...
                                {isExpanded
                                  ? t('audit.collapse', 'Daralt')
                                  : t('audit.expand', 'Genişlet')}
                              </span>
                            ...
                            {isExpanded && (
                              <div className="px-3 pb-3 pt-1 relative group/code">
                                ...
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    copyToClipboard(d._id, prettyJSON(d.metadata));
                                  }}
