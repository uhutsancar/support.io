...
import { Plus, Play, GitMerge, Save, X } from 'lucide-react';
...
const AutomationRules = () => {
  ...
  const toggleStatus = async (rule: any) => {
    try {
      const res = await api.put(`/automation-rules/${rule._id}`, {
        ...rule,
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            {t('automation.title', 'Automation Workflows')}
          </h1>
          <p className="text-gray-500 text-sm mt-1">
            {t('automation.subtitle', 'Automate routing, tagging, and replies')}
          </p>
        </div>
        <button
          onClick={() => openModal()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {rules.length === 0 ? (
          <div className="col-span-full py-12 text-center text-gray-500 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-dashed border-gray-300 dark:border-gray-700">
            ...
            <p>
              {t(
                'automation.emptyText',
                'No automation workflows yet. Create one to streamline your support.'
              )}
            </p>
          </div>
        ) : (
          rules.map((rule) => (
            <div
              key={rule._id}
              className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 flex flex-col transition-all hover:shadow-md"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`p-1 rounded-full ${rule.isActive ? 'text-green-600' : 'text-gray-400'}`}
                  >
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400 mb-4 flex-1">
                <div className="mb-2">
                  <span className="font-medium">{t('automation.triggerLabel', 'On:')}</span>{' '}
                  {rule.triggerType.replace('_', ' ')}
                </div>
                <div>
                  <span className="font-medium">{t('automation.actionsLabel', 'Actions:')}</span>{' '}
                  {rule.actions.length} {t('automation.actionsCount', 'action(s)')}
                </div>
              </div>

              <div className="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-700 text-sm">
                <span className="text-gray-500">
                  {rule.metrics?.executionsCount || 0} {t('automation.executions', 'executions')}
                </span>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50 rounded-t-xl">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                {editingRule
                  ? t('automation.editWorkflow', 'Edit Workflow')
                  : t('automation.newWorkflow', 'New Workflow')}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">
                    {t('automation.triggerLabelInput', 'Trigger')}
                  </label>
                  <select
                  ...
                  >
                    <option value="message_received">
                      {t('automation.triggerMsgReceived', 'Message Received')}
                    </option>
                    <option value="conversation_created">
                      {t('automation.triggerConvCreated', 'Conversation Created')}
                    </option>
                  </select>
                </div>
              </div>
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="font-semibold text-gray-900 dark:text-white">
                    {t('automation.ifConditions', 'IF Conditions')}
                  </h4>
                    ...
                    <div key={index} className="flex gap-2 items-center">
                      ...
                      >
                        <option value="message.content">
                          {t('automation.fieldMsgBody', 'Message Body')}
                        </option>
                        <option value="visitor.country">
                          {t('automation.fieldVisitorCountry', 'Visitor Country')}
                        </option>
                      </select>
                      <select
                        value={cond.operator}
              ...
              <div className="p-4 bg-indigo-50 dark:bg-indigo-900/10 rounded-xl border border-indigo-100 dark:border-indigo-800">
                <h4 className="font-semibold text-gray-900 dark:text-white mb-4">
                  {t('automation.thenActions', 'THEN Actions')}
                </h4>
                <div className="flex gap-2 items-end">
                  <div className="w-1/3">
                    <label className="block text-xs text-gray-500 mb-1">
                      {t('automation.actionType', 'Action Type')}
                    </label>
                    <select
                    ...
                    >
                      <option value="send_message">
                        {t('automation.actAutoReply', 'Auto Reply')}
                      </option>
                  ...
                  <div className="flex-1">
                    <label className="block text-xs text-gray-500 mb-1">
                      {t('automation.actionValue', 'Value')}
                    </label>
                    <input
                      type="text"
                      value={
                        formData.actions[0].payload.text || formData.actions[0].payload.tag || ''
                      }
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                      ...
                      placeholder={
                        formData.actions[0].type === 'add_tag'
                          ? 'urgent'
                          : t('automation.actAutoReplyHolder', 'Auto reply message')
                      }
                    />
                  </div>
                </div>
